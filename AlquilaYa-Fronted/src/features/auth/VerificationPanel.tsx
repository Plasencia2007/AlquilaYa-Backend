...
import { documentUploadSchema } from '@/validations/documentSchema';
...
export default function VerificationPanel() {
  ...
  const cargarDocumentos = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/usuarios/documentos/usuario/${usuario?.id}`);
      if (response.ok) {
  ...
  const handleFileUpload = async (tipo: TipoDoc, file: File) => {
    ...
    // 1. Validar con Zod
    const validation = documentUploadSchema.safeParse({ archivo: file, tipo: tipo as any });
    if (!validation.success) {
      const errorMsg = validation.error?.issues[0]?.message || "Archivo inválido";
      alert(`⚠️ Error de validación: ${errorMsg}`);
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:8080/api/v1/usuarios/documentos/upload', {
        method: 'POST',
        body: formData,
      });
