...
/** Paso de verificación de correo por código (modo Email), espejo del OtpStep de WhatsApp. */
export function EmailCodeStep() {
  ...
  const handleReenviar = async () => {
    ...
    setReenviando(true);
    try {
      await servicioAuth.reenviarVerificacionEmail(personal?.correo ?? '');
      notify.success('Código reenviado', 'Te enviamos un nuevo código a tu correo.');
      setCooldown(60);
    } catch (err) {
      notify.error(err, 'No se pudo reenviar el código');
    } finally {
      setReenviando(false);
    }
  };
    ...
    <div className="flex flex-col items-center space-y-6 text-center">
      ...
      <header className="space-y-2">
        ...
        <p className="mx-auto max-w-[280px] text-sm text-muted-foreground">
          ...
          <span className="font-bold text-foreground">{personal?.correo}</span>
        </p>
      </header>
